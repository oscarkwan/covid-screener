...
function Admin() {
  ...
  function updateRole(user, value) {
    const userCollectionUser = db.collection("userCollection").doc(user.uid);

    userCollectionUser.update({
      role: value,
    }).then(() => {
      ...
      {users ? (
        <Table data={users}>
          <Table.ColumnDefinition header="First and Last Name" cell={props => {
            return props.row.firstLastName;
          }} />
          ...
          <Table.ColumnDefinition header="Update Role" cell={props => {
            return (
              <>
                <OverflowMenu>
